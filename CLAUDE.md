...
- **Row visibility has one seam, and it is off.** *Enforced: `tests/services/test_tenancy_seam.py`.* `app/services/tenancy.py` answers "which rows may this User see"; `TENANCY_ENFORCED` ships `False`, and while it is off `scoped_select` returns the caller's statement untouched. That is what lets the ~40 read paths that never had an owner filter adopt the seam one batch at a time without any batch changing a response — ticket 21 flips the flag once an isolation guard proves it. The flag is read in **exactly one function**, `tenancy_enforced()`, and a guard greps the whole of `app/` to keep it that way: the failure mode of a flag is always the fourteenth place it got read, which is the same shape as the two auth gates that disagreed about `/password-recovery` for months.
  ...
  - **Every table is placed or excused, with a reason.** `SCOPES` classifies the 25 TG tables; `OUT_OF_SCOPE` says why `User`, `Item`, `Role`, and `UserRole` are not the seam's business. A table nobody classified fails the guard, which is the only moment when "whose rows are these?" is cheap to answer — and the guard walks subclasses **recursively**, because `User` and `Item` descend from `UserBase`/`ItemBase` and one level of `__subclasses__()` misses exactly those two. The follow-scoped branch **raises** rather than guessing while `tg_channel_follows` is missing: returning everything leaks, returning nothing is a silent outage, so flipping the flag early is a crash on the first query instead of a visibility bug found in production.
...
- **Scheduler runs in-process, single replica** (APScheduler), and the image therefore runs **`--workers 1`**. *Enforced: `tests/deployment/test_worker_count.py`.* This is a correctness constraint, not a capacity one: it shipped as `--workers 4` (template default, never reconciled with the scheduler) and each worker ran its own APScheduler — four `Auto Sync (scheduler)` jobs per tick, 4x every scheduled job, 711 rows stranded in `running`. Three things are per-process and must be externalised before the number can rise: the job registry (`scraper_jobs`), the per-proxy semaphores (`proxy_pool` — N workers means N x the request rate at Telegram), and the scheduler. The guard asserts all three, so **it fails when one is externalised** — that failure is the signal a step is done, not an obstacle. Sequenced plan: `docs/scaling-to-multiple-workers.md`. **Scale the API tier, never the sync tier.**
...
| `backend/tests/deployment/test_worker_count.py` | one worker, **and** the three reasons it has to be one | test |
