...
- **Four model modules, split by what the models *are*.** `models.py` (template auth: `User`), `models_tg.py` (all TG domain plus `AppSetting`), `models_rbac.py` (`Role`, `UserRole`), `models_view_as.py` (the View-as audit row). **Alembic's `env.py` must import every one.** A module it misses is invisible to autogenerate, which silently produces an *empty migration* rather than an error. The split is by category, not by count: a new category gets a new module, a model that fits an existing one does not.
...
- **Each widening is its own exchange on the Owner's own token, so a session can never widen itself.** *Enforced: `tests/api/test_view_as_elevation.py`, `tests/api/test_view_as_spend.py`.* `/elevate` authorises writes; `/spend` (ADR-017) authorises spending the **target's** AI Key, bots and Telegram Budget — a third tier, not a wider second, because a write is reversible and attributed and money that has left is neither. Each has its own Permission and a `Settings`-validated ceiling strictly shorter than the tier below, and each refuses a target holding **any** permission: "is an Admin" derived, not named. `VIEW_AS_SPEND_PATHS` carves the spending operations out of elevation **ahead of** the safe-method shortcut; `SPENDABLE_RESOURCES` is the exact set they name. `/view-as/*`, the credential routes and `/ai-keys` are refused at all three tiers, which `view_as_allows` alone answers. Attribution rides `session.info`, **not** a `contextvar` — `core/acting_owner.py` says why. `acted_by_*` is stamped on **every** write so an ordinary edit clears it, `LLMLog` included: a failed spend leaves no Artifact.
...
- **The unowned scope is gone, and the identity question it left behind is not the seam's.** *Enforced: `tests/services/test_setting_group_name_identity.py`.* Ticket 21 made all 14 owner columns `NOT NULL` with cascading keys, so `user_id=None` is an `IntegrityError` rather than "every account". The setting-group name filter mirrors its unique index, whose `COALESCE` is now a fossil left alone deliberately: a duplicate answers the route's 409 in **both** flag states and never a `UniqueViolation`. With the unowned leg gone the filter reads exactly like a scoped query, so the guard asserts the **reason** it is not one.
...
- **An Artifact is paid for by the Account that asked for it; everything else by the deployment.** *Enforced: `tests/services/test_ai_key_payment_rule.py`.* `services/ai_keys.py` is the sole writer of `tg_ai_credentials` and holds `resolve_ai_key`, the one answer to who pays — Summary, Chat, Tag and the model list run on the caller's Key; Embeddings, Translation and the RAG query embedding on the Operator Key, because each writes a row shared by every Follower (ADR-016). **No fallback in either direction** and no switch for one. `key_id` is client-supplied, so it is checked against the caller **before `decrypt_token`** — multi-user-tenancy ticket 33's defect under a new table. `AI_KEY_CALLERS` is a declared caller list walked from the AST. `get_provider` takes the credential as an argument and caches nothing.
- **Two Provider kinds and only two: Gemini, and anything OpenAI-compatible at a base URL.** *Enforced: `tests/api/test_openai_compatible_provider.py`.* One class reaches OpenRouter, Groq, DeepSeek, Ollama and vLLM alike, because they are the same API at different addresses; **there is no class per vendor**. The credential leaves in a header and never in a URL, because a recorded request body is composed at the call site and a key-bearing query parameter would land in one. **Both hardcoded model lists are gone**: `POST /ai/models` proxies the named Key's Provider, cached per Key, and an endpoint serving no catalogue answers `[]` so the client falls back to free text. It is a **POST because it spends the Account's own Key**, which is what puts it in `VIEW_AS_SPEND_PATHS`.
...
| `backend/tests/api/test_view_as_spend.py` | an elevation cannot spend; a safe method on a spend path still is one; the resource category is the exact set the inventory names | test |
...
| `backend/tests/services/test_setting_group_name_identity.py` | a duplicate group name is the route's 409 and never a `UniqueViolation`, in both flag states; two accounts may hold one name; the filter keeps no unowned leg and consults neither the flag nor `scoped_select` | test |
...
- **CI runs again** (2026-09-05). It was disabled from 2026-07-30 because GitHub-hosted minutes were billing-blocked on a private repo, which failed every job before its first step and left a permanently red `main` carrying no signal. Publishing the repo made those minutes free, so all six test workflows have their `push`/`pull_request` triggers back. Still run lint and tests locally before pushing: CI is now a second opinion, not the first one.
