...
- **An Artifact is paid for by the Account that asked for it; everything else by the deployment.** *Enforced: `tests/services/test_ai_key_payment_rule.py`.* `services/ai_keys.py` is the sole writer of `tg_ai_credentials` and holds `resolve_ai_key`, the one function that answers who pays — Summary, Chat and Tag run on the caller's AI Key; Embeddings, Translation and the RAG query embedding on the Operator Key, because each writes a row shared by every Follower (ADR-016). **No fallback in either direction** and no switch for one. `key_id` is client-supplied, so it is checked against the caller **before `decrypt_token`** — multi-user-tenancy ticket 33's defect under a new table. `AI_KEY_CALLERS` is a declared caller list walked from the AST, because a twelfth `get_provider` outside the seam still returns a Summary and charges the Operator silently. `get_provider` takes the credential as an argument and caches nothing; the old name-keyed cache handed one Account another's client.
- **Two Provider kinds and only two: Gemini, and anything OpenAI-compatible at a base URL.** *Enforced: `tests/api/test_openai_compatible_provider.py`.* One class reaches OpenRouter, Groq, Together, DeepSeek, Ollama and vLLM, because they are the same API at different addresses; **there is no class per vendor**. The credential leaves in a header and never in a URL, because a recorded request body is composed at the call site and a key-bearing query parameter would land in one. **Both hardcoded model lists are gone**, the backend's and `constants.ts`'s: `POST /ai/models` proxies the named Key's Provider, cached per Key, and an endpoint serving no catalogue answers `[]` so the client falls back to free text. It is a **POST and not on the View-as read-only allowlist** — it spends on an Account's behalf, which is the clause that already refuses `POST /rag/search`.
