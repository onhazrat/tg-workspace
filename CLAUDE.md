...
- **The write door is not the read door.** *Enforced: `tests/services/test_import_write_scoping.py`, `tests/services/test_auto_publish_scoping.py`.* `POST /data/import` and `/data/bot-credentials/migrate` reach rows by id and overwrite them; every table they write is checked or excused, derived from the AST. Import is per-account and stamps new rows with the **caller**, not the document. `publish_summary_text` refuses a foreign credential **before `decrypt_token`**, and files a failed publish log rather than returning quietly, because the scheduler is unattended.
