# #16 ⬆️ Upgrade backend dependencies to the template's floors

**State:** closed 2026-07-25 · **Branch:** `pr2/backend-deps` into `pr1/python-314` · **Diff:** +884 / -738 across 7 files · **Opened:** 2026-07-25

---

PR 2 of 7. **Stacked on #15** — review that first; base will retarget to `main` once #15 merges.
...
`ty` is held at `<0.0.55`, matching upstream's locked 0.0.54. ty 0.0.63 reports 7 diagnostics that both ty 0.0.54 **and** `mypy --strict` accept — six are the same sqlmodel scalar-select narrowing gap (`session.exec(select(Model.col)).all()` → `Sequence[str | Sequence[Unknown]]`). Annotating those sites is real work that doesn't belong in a dependency bump; it's tracked as follow-up and the pin carries a comment saying so.
...
This was the migration's highest-consequence unknown — `BaseHTTPMiddleware` + streaming is the historically fragile combination, and six endpoints depend on it. Probed with the real `APIKeyMiddleware` plus an `@app.middleware("http")` decorator, in `main.py`'s ordering:
...
Five 250 ms yields arriving spread across 1.26 s — genuinely streamed, not buffered to EOF.
...
starlette 1.x deprecates using `httpx` with `starlette.testclient` in favour of `httpx2`. Warning only, not urgent.
