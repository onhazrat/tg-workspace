...
- **Command palette:** `Cmd+Shift+P` / `Ctrl+Shift+P` + header icon; `CommandPaletteProvider` in `TgProviders`. Settings + job toggles + navigate + channel entity flows + channel ops + data transfer + in-palette search. Recents when search empty; query→command affinity (`localStorage`); `VITE_COMMAND_PALETTE_RECENT_COUNT` (default 5). Entity sub-flows: `closeOnPick: false` for select/deselect/freeze/unfreeze/toggle-auto-follow (multi-pick stays open). Tag search: `tag:` / `#` prefix via `filter-channels.ts`. Post/summary search: in-palette `search-results` mode (cap 50); empty query clears filter; post search clears semantic. Entity input refocus via `requestAnimationFrame`. `requiresConfirmation` on bulk freeze/unfreeze, delete channel, import, clear cache. **Keyboard UX (IDEA-007, 2026-06-25):** shared `usePaletteListSelection` for controlled cmdk `value`/`onValueChange` in `commands` | `entity` | `search-results`; `loop={true}` on all list modes; `PaletteSubViewHeader` / `PaletteFooterHints`; editor `↵` / `⌃↵` hints + `isEditorApplying` blocks async semantic search; dialog close `tabIndex={-1}` in list modes; `onOpenAutoFocus` preventDefault; E2E keyboard suite K1–K11 in `summarizer.spec.ts`. Confirm dialog keyboard handled separately in `CommandConfirmDialog.tsx`. **Keyboard UX (IDEA-007, 2026-06-25):** shared `usePaletteListSelection` for commands/entity/search-results; `loop={true}`; confirm autofocus Cancel + Enter/arrows; editor `isEditorApplying` blocks async Enter; footer hints; dialog X `tabIndex={-1}` in list modes; Playwright K1–K11 keyboard-only in `summarizer.spec.ts`. **Keyboard UX (IDEA-007):** `usePaletteListSelection` controlled cmdk on commands/entity/search-results with `loop={true}`; `PaletteSubViewHeader` + `PaletteFooterHints`; confirm — Cancel autofocus, Enter on focused button, ←→↑↓; editor Apply `data-testid`, Enter/⌃↵ apply, disabled while async search pending; close button `tabIndex={-1}` in list modes; Playwright K1–K11 (`summarizer.spec.ts`). Audit: [IDEA-007](docs/ideas-log/ideas/IDEA-007-command-palette-keyboard-ux.md).
...
22. **Palette keyboard UX (IDEA-007, 2026-06-25)** — `usePaletteListSelection` shared hook; controlled selection in all list modes; `loop={true}`; editor footer hints + async apply guard; dialog X removed from tab order in list modes; keyboard-only E2E K1–K11. Confirm dialog keyboard in `CommandConfirmDialog.tsx` (parallel). Import after confirm still requires native file picker (documented exception).
...
- **Ideas backlog** — [IDEAS-LOG.md](docs/ideas-log/IDEAS-LOG.md): **IDEA-001/004/005 implemented locally (2026-06-24), pending commit + E2E verify**; **IDEA-007 palette keyboard UX implemented (2026-06-25)**; [IDEA-002](docs/ideas-log/ideas/IDEA-002-tanstack-devtools.md) TanStack devtools (dev-only).
