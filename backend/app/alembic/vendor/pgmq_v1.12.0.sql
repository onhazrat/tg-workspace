------------------------------------------------------------
-- Vendored, unmodified, from https://github.com/pgmq/pgmq
-- tag v1.12.0, path pgmq-extension/sql/pgmq.sql
-- PostgreSQL License (github.com/pgmq/pgmq/blob/v1.12.0/LICENSE)
--
-- This is PGMQ's *pure-SQL* install path (ticket 09 /
-- docs/multi-user-tenancy-plan.md decision 25): plain `CREATE TABLE` /
-- `CREATE FUNCTION` statements the app's own DB role can run, not
-- `CREATE EXTENSION pgmq`. That keeps `postgres:18` stock (no image change to
-- bake the compiled extension in) and needs no superuser (`CREATE EXTENSION`
-- does; plain DDL owned by the role that creates it does not). The functions
-- that require `pg_partman` (partitioned queues) are still defined here but
-- never called — `pgmq.create()` (non-partitioned) is what
-- `app/services/pgmq.py` and its migration use.
--
-- Do not hand-edit below this line. Re-vendor the whole file from a newer tag
-- instead, so a diff against upstream stays meaningful.
------------------------------------------------------------
