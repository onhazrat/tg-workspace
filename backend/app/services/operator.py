...
def distinct_operator_setting_group_ids(
    session: Session,
    *,
    operator_id: uuid.UUID | None = None,
) -> set[str]:
    """Return distinct setting_group_id values for operator-scoped channels (SQL only)."""
    if operator_id is None:
        operator_id = get_operator_user_id(session)
    ...
    if (
        ...
        logger.info(
            "Mode A local fallback: no operator-scoped setting groups; using all channels"
        )
        group_ids = _distinct_ids(None)
