...
export const LogsView: React.FC = () => {
  const { publishLogs, loadLogs, syncLogs, loadSyncLogs, llmLogs, loadLLMLogs, networkLogs, loadNetworkLogs, embeddingLogs, loadEmbeddingLogs } = useData();
  ...
  const [expandedPublishLog, setExpandedPublishLog] = useState<string | null>(null);
  const [expandedSyncLog, setExpandedSyncLog] = useState<string | null>(null);
  const [expandedNetworkLog, setExpandedNetworkLog] = useState<string | null>(null);
  const [expandedEmbeddingLog, setExpandedEmbeddingLog] = useState<string | null>(null);
  ...
  }, [activeLogTab, searchQuery, statusFilter, startDate, endDate, modelFilter, botFilter, channelFilter, searchInDetails]);
  ...
  }, [activeLogTab, observerTarget.current]);
  ...
  const filteredPublishLogs = useMemo(() => {
    return publishLogs.filter(log => {
      ...
      if (searchQuery) {
        ...
        if (searchInDetails) {
          const reqStr = log.fullRequest ? JSON.stringify(log.fullRequest).toLowerCase() : "";
          const resStr = log.fullResponse ? JSON.stringify(log.fullResponse).toLowerCase() : "";
          matchesDetails = reqStr.includes(q) || resStr.includes(q);
        }

        if (!matchesBot && !matchesChat && !matchesChatId && !matchesError && !matchesText && !matchesDetails) return false;
      }
      return true;
    });
  }, [publishLogs, statusFilter, searchQuery, startDate, endDate, botFilter, searchInDetails]);
  ...
  const filteredSyncLogs = useMemo(() => {
    return syncLogs.filter(log => {
      ...
      // Channel filter
      if (channelFilter !== "all" && log.channelName !== channelFilter) return false;
      ...
      if (searchQuery) {
        ...
        if (!matchesChannel && !matchesSource && !matchesError && !matchesDetails) return false;
      }
      return true;
    });
  }, [syncLogs, statusFilter, searchQuery, startDate, endDate, channelFilter, searchInDetails]);
  ...
  const filteredLlmLogs = useMemo(() => {
    return llmLogs.filter(log => {
      ...
      if (searchQuery) {
        ...
        if (!matchesModel && !matchesPrompt && !matchesResponse && !matchesError && !matchesDetails) return false;
      }
      return true;
    });
  }, [llmLogs, statusFilter, searchQuery, startDate, endDate, modelFilter, searchInDetails]);
  ...
  const filteredNetworkLogs = useMemo(() => {
    return networkLogs.filter(log => {
      ...
      if (searchQuery) {
        ...
        let matchesDetails = false;
        if (searchInDetails) {
          const telStr = log.telemetry ? JSON.stringify(log.telemetry).toLowerCase() : "";
          matchesDetails = telStr.includes(q);
        }
        if (!matchesUrl && !matchesMethod && !matchesError && !matchesDetails) return false;
      }
      return true;
    });
  }, [networkLogs, statusFilter, searchQuery, startDate, endDate, searchInDetails]);
  ...
  const isAnyFilterActive = useMemo(() => {
    return searchQuery !== "" || 
           ...
           channelFilter !== "all" ||
           searchInDetails;
  }, [searchQuery, statusFilter, startDate, endDate, modelFilter, botFilter, channelFilter, searchInDetails]);
    ...
    >
      <div className="flex flex-col gap-4 mb-6">
        <div className="flex justify-between items-end">
          <div className="text-left">
            <div className="flex items-baseline gap-3">
              <h3 className="text-sm uppercase font-bold tracking-widest">System Logs</h3>
              <span className="text-[10px] font-mono opacity-40">
                [{activeLogTab === "publish" ? filteredPublishLogs.length : activeLogTab === "sync" ? filteredSyncLogs.length : activeLogTab === "llm" ? filteredLlmLogs.length : activeLogTab === "network" ? filteredNetworkLogs.length : filteredEmbeddingLogs.length} RECORDS]
              </span>
            </div>
          ...
          <div className="flex items-center gap-2">
            <button 
              onClick={activeLogTab === "publish" ? handleClearPublishLogs : activeLogTab === "sync" ? handleClearSyncLogs : activeLogTab === "llm" ? handleClearLlmLogs : activeLogTab === "network" ? handleClearNetworkLogs : handleClearEmbeddingLogs}
            ...
            >
              <Trash2 size={12} />
              Clear {activeLogTab === "publish" ? "Publish" : activeLogTab === "sync" ? "Sync" : activeLogTab === "llm" ? "LLM" : activeLogTab === "network" ? "Network" : "Embedding"} Logs
            </button>
          </div>
        </div>
        ...
        <div className="flex border-b border-app-ink/10">
          <button
            onClick={() => { setActiveLogTab("publish"); setSearchQuery(""); setStatusFilter("all"); }}
            className={`px-4 py-2 text-[10px] uppercase font-bold tracking-widest transition-all border-b-2 ${
              activeLogTab === "publish" ? "border-app-ink opacity-100" : "border-transparent opacity-40"
            }`}
          >
            Publish Logs
          </button>
          <button
            onClick={() => { setActiveLogTab("sync"); setSearchQuery(""); setStatusFilter("all"); }}
            className={`px-4 py-2 text-[10px] uppercase font-bold tracking-widest transition-all border-b-2 ${
              activeLogTab === "sync" ? "border-app-ink opacity-100" : "border-transparent opacity-40"
            }`}
          >
            Sync Logs
          </button>
          <button
            onClick={() => { setActiveLogTab("llm"); setSearchQuery(""); setStatusFilter("all"); }}
            className={`px-4 py-2 text-[10px] uppercase font-bold tracking-widest transition-all border-b-2 ${
              activeLogTab === "llm" ? "border-app-ink opacity-100" : "border-transparent opacity-40"
            }`}
          >
            LLM Logs
          </button>
          <button
            onClick={() => { setActiveLogTab("network"); setSearchQuery(""); setStatusFilter("all"); }}
            className={`px-4 py-2 text-[10px] uppercase font-bold tracking-widest transition-all border-b-2 ${
              activeLogTab === "network" ? "border-app-ink opacity-100" : "border-transparent opacity-40"
            }`}
          >
            Network Logs
          </button>
          <button
            onClick={() => { setActiveLogTab("embedding"); setSearchQuery(""); setStatusFilter("all"); }}
            className={`px-4 py-2 text-[10px] uppercase font-bold tracking-widest transition-all border-b-2 ${
              activeLogTab === "embedding" ? "border-app-ink opacity-100" : "border-transparent opacity-40"
            }`}
          >
            Embedding Logs
          </button>
        ...
        <div className="flex gap-2">
          <div className="flex-1 flex gap-2">
            <div className="relative flex-1">
              ...
              <input 
                type="text"
                placeholder={activeLogTab === "publish" ? "SEARCH PUBLISH LOGS..." : activeLogTab === "sync" ? "SEARCH SYNC LOGS..." : activeLogTab === "llm" ? "SEARCH LLM LOGS..." : activeLogTab === "network" ? "SEARCH NETWORK LOGS..." : "SEARCH EMBEDDING LOGS..."}
                value={searchQuery}
            ...
            <div className="flex items-center gap-2 px-3 border border-app-ink/10 bg-app-ink/5">
              ...
              />
              <label htmlFor="search-details" className="text-[8px] uppercase font-bold opacity-50 cursor-pointer select-none">
                Search in Details
              </label>
            </div>
          </div>
              ...
              <div className="p-4 bg-app-ink/5 border border-app-ink/10 space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  <div className="space-y-2">
                    <label className="text-[9px] uppercase font-bold opacity-50">Status</label>
                    <div className="flex gap-2">
                  ...
                  <div className="space-y-2 col-span-1 md:col-span-2">
                    <label className="text-[9px] uppercase font-bold opacity-50">Date Range</label>
                    <div className="flex items-center gap-3">
                      <input 
                        type="date"
                        value={startDate && !isNaN(startDate) ? formatDateToLocalISO(new Date(startDate)).split('T')[0] : ""}
                        max={formatDateToLocalISO(new Date()).split('T')[0]}
                        onChange={(e) => {
                          if (e.target.value) {
                            const time = new Date(e.target.value + "T00:00:00").getTime();
                            setStartDate(isNaN(time) ? null : time);
                          } else {
                            setStartDate(null);
                          }
                        }}
                      ...
                      <input 
                        type="date"
                        value={endDate && !isNaN(endDate) ? formatDateToLocalISO(new Date(endDate)).split('T')[0] : ""}
                        max={formatDateToLocalISO(new Date()).split('T')[0]}
                        onChange={(e) => {
                          if (e.target.value) {
                            const time = new Date(e.target.value + "T00:00:00").getTime();
                            setEndDate(isNaN(time) ? null : time);
                          } else {
                            setEndDate(null);
                          }
                        }}
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 pt-4 border-t border-app-ink/5">
                  ...
                  {activeLogTab === "llm" && (
                    <div className="space-y-2">
                      <label className="text-[9px] uppercase font-bold opacity-50">Model</label>
                      <select 
                        value={modelFilter}
                      ...
                      >
                        ...
                        {uniqueModels.map(m => <option key={m} value={m}>{m.toUpperCase()}</option>)}
                      </select>
                    </div>
                  )}
                  ...
                  {activeLogTab === "publish" && (
                    <div className="space-y-2">
                      <label className="text-[9px] uppercase font-bold opacity-50">Bot</label>
                      <select 
                        value={botFilter}
                      ...
                      >
                        ...
                        {uniqueBots.map(b => <option key={b} value={b}>{b.toUpperCase()}</option>)}
                      </select>
                    </div>
                  )}
                  ...
                  {activeLogTab === "sync" && (
                    <div className="space-y-2">
                      <label className="text-[9px] uppercase font-bold opacity-50">Channel</label>
                      <select 
                        value={channelFilter}
                      ...
                      >
                        ...
                        {uniqueChannels.map(c => <option key={c} value={c}>{c.toUpperCase()}</option>)}
                      </select>
                    </div>
                  )}
        ...
        {activeLogTab === "publish" ? (
          filteredPublishLogs.length === 0 ? (
            <div className="py-20 text-center border border-dashed border-app-ink/10 opacity-30">
              <p className="text-[10px] uppercase font-mono tracking-[0.2em]">No publish logs found</p>
            </div>
          ) : (
            filteredPublishLogs.slice(0, visiblePublishLogs).map((log) => (
              ...
              >
                <div className="flex justify-between items-start mb-3">
                  <div className="flex items-center gap-3">
                    <div className={`p-2 rounded-full ${log.status === "success" ? "bg-green-500/10 text-green-600" : "bg-red-500/10 text-red-600"}`}>
                      {log.status === "success" ? <CheckCircle2 size={16} /> : <XCircle size={16} />}
                    </div>
                    <div>
                      <div className="flex items-center gap-2">
                        <span className="text-[10px] font-mono font-bold uppercase tracking-widest">
                          {log.status === "success" ? "Published Successfully" : "Publish Failed"}
                        </span>
                        <span className="text-[9px] opacity-30 font-mono"><RelativeTime timestamp={log.timestamp} /></span>
                      </div>
                      <div className="flex items-center gap-4 mt-1">
                        ...
                        <div className="flex items-center gap-1.5 text-[9px] font-bold uppercase opacity-60">
                          <Send size={10} />
                          <span>Dest: {log.chatName} ({log.chatId})</span>
                        </div>
                      </div>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <button 
                          onClick={() => setExpandedPublishLog(expandedPublishLog === log.id ? null : log.id)}
                          className="p-1.5 text-app-ink opacity-30 hover:opacity-100 transition-all border border-app-ink/10 hover:bg-app-ink/5"
                        >
                          <ChevronDown size={14} className={`transition-transform ${expandedPublishLog === log.id ? "rotate-180" : ""}`} />
                        </button>
                      </TooltipTrigger>
                      <TooltipContent>
                        <p>{expandedPublishLog === log.id ? "Collapse" : "Expand"} Details</p>
                      </TooltipContent>
                    </Tooltip>
                    <Tooltip>
                      <TooltipTrigger asChild>
                ...
                {log.status === "failed" && log.error && (
                  <div className="mt-3 p-3 bg-red-500/5 border border-red-500/10 rounded flex items-start gap-3">
                    ...
                    <div className="space-y-1">
                      <p className="text-[9px] font-bold uppercase text-red-500 opacity-70">Error Message:</p>
                ...
                <div className="mt-3 pt-3 border-t border-app-ink/5">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="text-[8px] font-bold uppercase opacity-40">Summary ID:</span>
                    <span className="text-[8px] font-mono opacity-60">{log.summaryId}</span>
                  </div>
                </div>
                    ...
                    >
                      <div className="mt-4 space-y-4 pt-4 border-t border-app-ink/5">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          {log.fullRequest && (
                            <div className="space-y-1.5">
                              <label className="text-[8px] uppercase font-bold opacity-40">Full Request (JSON)</label>
                          ...
                          {log.fullResponse && (
                            <div className="space-y-1.5">
                              <label className="text-[8px] uppercase font-bold opacity-40">Full Response (JSON)</label>
                              <div className="p-3 bg-app-ink/5 border border-app-ink/10 rounded font-mono text-[9px] whitespace-pre overflow-x-auto max-h-60 opacity-60">
          ...
          filteredSyncLogs.length === 0 ? (
            <div className="py-20 text-center border border-dashed border-app-ink/10 opacity-30">
              <p className="text-[10px] uppercase font-mono tracking-[0.2em]">No sync logs found</p>
            </div>
          ) : (
            filteredSyncLogs.slice(0, visibleSyncLogs).map((log) => (
              <div 
                key={log.id}
                className={`group border border-app-ink border-opacity-10 p-4 transition-all hover:border-opacity-30 bg-app-card ${
                  log.status === "failed" ? "border-l-4 border-l-red-500" : "border-l-4 border-l-blue-500"
                }`}
              >
                <div className="flex justify-between items-start mb-3">
                  <div className="flex items-center gap-3">
                    <div className={`p-2 rounded-full ${log.status === "success" ? "bg-blue-500/10 text-blue-600" : "bg-red-500/10 text-red-600"}`}>
                      {log.status === "success" ? <RefreshCw size={16} /> : <XCircle size={16} />}
                    </div>
                    <div>
                      <div className="flex items-center gap-2">
                        <span className="text-[10px] font-mono font-bold uppercase tracking-widest">
                  ...
                  <div className="flex items-center gap-2">
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <button 
                          onClick={() => setExpandedSyncLog(expandedSyncLog === log.id ? null : log.id)}
                          className="p-1.5 text-app-ink opacity-30 hover:opacity-100 transition-all border border-app-ink/10 hover:bg-app-ink/5"
                        >
                          <ChevronDown size={14} className={`transition-transform ${expandedSyncLog === log.id ? "rotate-180" : ""}`} />
                        </button>
                      </TooltipTrigger>
                      <TooltipContent>
                        <p>{expandedSyncLog === log.id ? "Collapse" : "Expand"} Details</p>
                      </TooltipContent>
                    </Tooltip>
                    <Tooltip>
                      <TooltipTrigger asChild>
          ...
          filteredLlmLogs.length === 0 ? (
            <div className="py-20 text-center border border-dashed border-app-ink/10 opacity-30">
              <p className="text-[10px] uppercase font-mono tracking-[0.2em]">No LLM logs found</p>
            </div>
          ) : (
            filteredLlmLogs.slice(0, visibleLlmLogs).map((log) => (
              <div 
                key={log.id}
                className={`group border border-app-ink border-opacity-10 p-4 transition-all hover:border-opacity-30 bg-app-card ${
                  log.status === "failed" ? "border-l-4 border-l-red-500" : "border-l-4 border-l-purple-500"
                }`}
              >
                <div className="flex justify-between items-start mb-3">
                  <div className="flex items-center gap-3">
                    <div className={`p-2 rounded-full ${log.status === "success" ? "bg-purple-500/10 text-purple-600" : "bg-red-500/10 text-red-600"}`}>
                      {log.status === "success" ? <Cpu size={16} /> : <XCircle size={16} />}
                    </div>
                    <div>
                      <div className="flex items-center gap-2">
                        <span className="text-[10px] font-mono font-bold uppercase tracking-widest">
                          {log.status === "success" ? "LLM Interaction" : "LLM Error"}
                        </span>
                        <span className="text-[9px] opacity-30 font-mono"><RelativeTime timestamp={log.timestamp} /></span>
                      </div>
                      <div className="flex items-center gap-4 mt-1">
                        ...
                        <div className="flex items-center gap-1.5 text-[9px] font-bold uppercase opacity-60">
                          {log.type === "chat" ? <MessageSquare size={10} /> : <FileText size={10} />}
                  ...
                  <div className="flex items-center gap-2">
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <button 
                          onClick={() => setExpandedLlmLog(expandedLlmLog === log.id ? null : log.id)}
                          className="p-1.5 text-app-ink opacity-30 hover:opacity-100 transition-all border border-app-ink/10 hover:bg-app-ink/5"
                        >
                          <ChevronDown size={14} className={`transition-transform ${expandedLlmLog === log.id ? "rotate-180" : ""}`} />
                        </button>
                      </TooltipTrigger>
                      <TooltipContent>
                        <p>{expandedLlmLog === log.id ? "Collapse" : "Expand"} Details</p>
                      </TooltipContent>
                    </Tooltip>
                    <Tooltip>
                      <TooltipTrigger asChild>
                      ...
                      <div className="mt-4 space-y-4 pt-4 border-t border-app-ink/5">
                        <div className="flex flex-wrap gap-4 mb-4">
                          {log.tokens && (
                            <div className="flex flex-col">
                              <span className="text-[7px] font-mono uppercase tracking-widest opacity-30 mb-0.5">Total Tokens</span>
                              <span className="text-[10px] font-bold tracking-tighter leading-none font-mono">{log.tokens}</span>
                            </div>
                          )}
                          {log.modelConfig && (
                            <div className="flex flex-col">
                              <span className="text-[7px] font-mono uppercase tracking-widest opacity-30 mb-0.5">Temperature</span>
                              <span className="text-[10px] font-bold tracking-tighter leading-none font-mono">{log.modelConfig.temperature}</span>
                            </div>
                          )}
                        </div>
                        ...
                        <div className="space-y-1.5">
                          ...
                          <div className="p-3 bg-app-ink/5 border border-app-ink/10 rounded font-mono text-[10px] whitespace-pre-wrap max-h-60 overflow-y-auto leading-relaxed">
                            {log.response || (log.status === "failed" ? "No response generated." : "Empty response.")}
                          </div>
                        </div>
          ...
          filteredNetworkLogs.length === 0 ? (
            <div className="py-20 text-center border border-dashed border-app-ink/10 opacity-30">
              <p className="text-[10px] uppercase font-mono tracking-[0.2em]">No network logs found</p>
            </div>
          ) : (
            filteredNetworkLogs.slice(0, visibleNetworkLogs).map((log) => (
              <div 
                key={log.id}
                className={`group border border-app-ink border-opacity-10 p-4 transition-all hover:border-opacity-30 bg-app-card ${
                  log.status === "failed" ? "border-l-4 border-l-red-500" : "border-l-4 border-l-teal-500"
                }`}
              >
                <div className="flex justify-between items-start mb-3">
                  <div className="flex items-center gap-3">
                    <div className={`p-2 rounded-full ${log.status === "success" ? "bg-teal-500/10 text-teal-600" : "bg-red-500/10 text-red-600"}`}>
                      {log.status === "success" ? <Globe size={16} /> : <XCircle size={16} />}
                    </div>
                    <div>
                      <div className="flex items-center gap-2">
                        <span className="text-[10px] font-mono font-bold uppercase tracking-widest">
                          {log.status === "success" ? "Network Request" : "Network Error"}
                        </span>
                        <span className="text-[9px] opacity-30 font-mono"><RelativeTime timestamp={log.timestamp} /></span>
                      </div>
                      <div className="flex items-center gap-4 mt-1">
                        ...
                        <div className="flex items-center gap-1.5 text-[9px] font-bold uppercase opacity-60">
                          <Activity size={10} />
                          <span>{log.proxyUsed ? (log.proxyUsed.includes('socks5h://127.0.0.1') ? 'Tor' : 'Proxy') : 'Direct'}</span>
                        </div>
                  ...
                  <div className="flex items-center gap-2">
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <button 
                          onClick={() => setExpandedNetworkLog(expandedNetworkLog === log.id ? null : log.id)}
                          className="p-1.5 text-app-ink opacity-30 hover:opacity-100 transition-all border border-app-ink/10 hover:bg-app-ink/5"
                        >
                          <ChevronDown size={14} className={`transition-transform ${expandedNetworkLog === log.id ? "rotate-180" : ""}`} />
                        </button>
                      </TooltipTrigger>
                      <TooltipContent>
                        <p>{expandedNetworkLog === log.id ? "Collapse" : "Expand"} Details</p>
                      </TooltipContent>
                    </Tooltip>
                    <Tooltip>
                      <TooltipTrigger asChild>
                  ...
                  {expandedNetworkLog === log.id && (
                    ...
                    >
                      <div className="mt-4 space-y-4 pt-4 border-t border-app-ink/5">
                        {log.telemetry && (
                          <div className="space-y-1.5">
                            <label className="text-[8px] uppercase font-bold opacity-40">Telemetry Data</label>
                            <div className="p-3 bg-app-ink/5 border border-app-ink/10 rounded font-mono text-[9px] whitespace-pre overflow-x-auto max-h-60 opacity-60">
          ...
          filteredEmbeddingLogs.length === 0 ? (
            <div className="py-20 text-center border border-dashed border-app-ink/10 opacity-30">
              <p className="text-[10px] uppercase font-mono tracking-[0.2em]">No embedding logs found</p>
            </div>
          ) : (
            filteredEmbeddingLogs.slice(0, visibleEmbeddingLogs).map((log) => (
              <div 
                key={log.id}
                className={`group border border-app-ink border-opacity-10 p-4 transition-all hover:border-opacity-30 bg-app-card ${
                  log.status === "failed" ? "border-l-4 border-l-red-500" : "border-l-4 border-l-orange-500"
                }`}
              >
                <div className="flex justify-between items-start mb-3">
                  <div className="flex items-center gap-3">
                    <div className={`p-2 rounded-full ${log.status === "success" ? "bg-orange-500/10 text-orange-600" : "bg-red-500/10 text-red-600"}`}>
                      {log.status === "success" ? <Hash size={16} /> : <XCircle size={16} />}
                    </div>
                    <div>
                      <div className="flex items-center gap-2">
                        <span className="text-[10px] font-mono font-bold uppercase tracking-widest">
                          {log.status === "success" ? "Embedding Generation" : "Embedding Error"}
                        </span>
                        <span className="text-[9px] opacity-30 font-mono"><RelativeTime timestamp={log.timestamp} /></span>
                      </div>
                      <div className="flex items-center gap-4 mt-1">
                  ...
                  <div className="flex items-center gap-2">
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <button 
                          onClick={() => setExpandedEmbeddingLog(expandedEmbeddingLog === log.id ? null : log.id)}
                          className="p-1.5 text-app-ink opacity-30 hover:opacity-100 transition-all border border-app-ink/10 hover:bg-app-ink/5"
                        >
                          <ChevronDown size={14} className={`transition-transform ${expandedEmbeddingLog === log.id ? "rotate-180" : ""}`} />
                        </button>
                      </TooltipTrigger>
                      <TooltipContent>
                        <p>{expandedEmbeddingLog === log.id ? "Collapse" : "Expand"} Details</p>
                      </TooltipContent>
                    </Tooltip>
                    <Tooltip>
                      <TooltipTrigger asChild>
