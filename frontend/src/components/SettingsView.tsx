...
import { Settings, Sun, Moon, Globe, Languages, Cpu, RefreshCw, Thermometer, Layout, Info, Activity, Shield, CheckCircle2, XCircle, Zap, RotateCw, Database } from "lucide-react";
...
export const SettingsView: React.FC<{ activeSection?: string }> = ({ activeSection = "preferences" }) => {
  ...
  const fetchProxyHealth = async () => {
    try {
      ...
      setBadProxies(data.badProxies as { url: string; cooldownRemaining: number }[]);
    } catch (error) {
  ...
  const testProxy = async (proxyUrl: string) => {
    ...
    try {
      const data = await api.testProxy(proxyUrl) as { success?: boolean; error?: string };
      ...
      setProxyTestResults(prev => ({ ...prev, [proxyUrl]: { success: false, error: error.message } }));
    } finally {
          ...
          <div className="text-left">
            <div className="flex items-baseline gap-3">
              <h3 className="text-sm uppercase font-bold tracking-widest">
                {activeSection === "appearance" ? "System Configuration" : 
                 activeSection === "sync" ? "Scraping & Synchronization" :
                 activeSection === "ai" ? "AI & Models" : 
                 "Network Configuration"}
              </h3>
              <span className="text-[10px] font-mono opacity-40">[{activeSection.toUpperCase()}]</span>
            </div>
            <p className="text-[10px] italic serif opacity-50 mt-1">
              {activeSection === "appearance" ? "Adjust appearance and interface settings." : 
               activeSection === "sync" ? "Configure automation, schedules, and data sync." :
               activeSection === "ai" ? "Configure LLMs, embeddings, and generation parameters." : 
               "Manage proxies, TOR, and synchronization networks."}
            </p>
          </div>
          
          <div className="flex items-center gap-2">
            <span className="text-[10px] font-bold uppercase tracking-tight opacity-60">Advanced Mode</span>
            <button 
              onClick={() => setAdvancedMode(!advancedMode)}
              className={`w-10 h-5 transition-all relative border border-app-ink/20 ${advancedMode ? 'bg-app-ink' : 'bg-app-ink/10'}`}
            >
              <div className={`absolute top-0.5 w-3.5 h-3.5 transition-all ${advancedMode ? 'left-5.5 bg-app-bg' : 'left-0.5 bg-app-ink/50'}`}></div>
            </button>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        {activeSection === "appearance" && (
          <div className="space-y-8 lg:col-span-2">
            <div className="bg-app-card border border-app-ink/10 p-6 shadow-sm">
              <div className="flex items-center gap-3 mb-6">
                ...
                <h4 className="text-[11px] uppercase font-bold tracking-widest">Interface & Appearance</h4>
              </div>
            ...
            <div className="space-y-8">
              <div className="space-y-4">
                <div className="flex items-center gap-2 opacity-60">
                  <Sun size={14} />
                  <span className="text-[10px] font-bold uppercase tracking-tight">Color Theme</span>
                </div>
                <div className="flex gap-2 p-1 bg-app-ink/5 border border-app-ink/10 w-fit">
                  <button
                    ...
                    className={`px-4 py-2 text-[10px] font-bold uppercase tracking-widest transition-all flex items-center gap-2 ${
                      theme === "light" ? "bg-app-ink text-app-bg" : "opacity-40 hover:opacity-100"
                    }`}
                  >
                    <Sun size={12} /> Light
                  </button>
                  <button
                    ...
                    className={`px-4 py-2 text-[10px] font-bold uppercase tracking-widest transition-all flex items-center gap-2 ${
                      theme === "dark" ? "bg-app-ink text-app-bg" : "opacity-40 hover:opacity-100"
                    }`}
                  >
                    <Moon size={12} /> Dark
                  </button>
            ...
            <div className="bg-app-card border border-app-ink/10 p-6 shadow-sm">
              <div className="flex items-center gap-3 mb-6">
                ...
                <h4 className="text-[11px] uppercase font-bold tracking-widest">System Information</h4>
              </div>
              
              <div className="space-y-6">
                <p className="text-[10px] leading-relaxed opacity-60 italic serif">
                  This dashboard is designed for high-speed monitoring and analysis of Telegram channels. 
                  All data is stored locally in your browser's IndexedDB. AI processing is powered by 
                  Google Gemini. No data is sent to external servers except for Telegram scraping and AI analysis.
                </p>
          ...
          <div className="bg-app-card border border-app-ink/10 p-6 shadow-sm mt-8">
            <div className="flex items-center gap-3 mb-6">
              <Layout size={18} className="opacity-40" />
              <h4 className="text-[11px] uppercase font-bold tracking-widest">Channel Card Display</h4>
            </div>
            ...
            <div className="space-y-6">
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2 opacity-60">
                    <span className="text-[10px] font-bold uppercase tracking-tight">Show Bio</span>
                  </div>
                  <button 
                  ...
                  >
                    <div className={`absolute top-0.5 w-3.5 h-3.5 bg-white transition-all ${showChannelBio ? 'left-5.5' : 'left-0.5'}`}></div>
                  </button>
                </div>
              </div>
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2 opacity-60">
                    <span className="text-[10px] font-bold uppercase tracking-tight">Show Subscribers</span>
                  </div>
                  <button 
                    onClick={() => setShowChannelSubscribers(!showChannelSubscribers)}
                  ...
                  >
                    <div className={`absolute top-0.5 w-3.5 h-3.5 bg-white transition-all ${showChannelSubscribers ? 'left-5.5' : 'left-0.5'}`}></div>
                  </button>
                </div>
              </div>
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2 opacity-60">
                    <span className="text-[10px] font-bold uppercase tracking-tight">Show Photos Count</span>
                  </div>
                  <button 
                  ...
                  >
                    <div className={`absolute top-0.5 w-3.5 h-3.5 bg-white transition-all ${showChannelPhotos ? 'left-5.5' : 'left-0.5'}`}></div>
                  </button>
                </div>
              </div>
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2 opacity-60">
                    <span className="text-[10px] font-bold uppercase tracking-tight">Show Videos Count</span>
                  </div>
                  <button 
                  ...
                  >
                    <div className={`absolute top-0.5 w-3.5 h-3.5 bg-white transition-all ${showChannelVideos ? 'left-5.5' : 'left-0.5'}`}></div>
                  </button>
                </div>
              </div>
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2 opacity-60">
                    <span className="text-[10px] font-bold uppercase tracking-tight">Show Files Count</span>
                  </div>
                  <button 
                  ...
                  >
                    <div className={`absolute top-0.5 w-3.5 h-3.5 bg-white transition-all ${showChannelFiles ? 'left-5.5' : 'left-0.5'}`}></div>
                  </button>
                </div>
              </div>
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2 opacity-60">
                    <span className="text-[10px] font-bold uppercase tracking-tight">Show Links Count</span>
                  </div>
                  <button 
                  ...
                  >
                    <div className={`absolute top-0.5 w-3.5 h-3.5 bg-white transition-all ${showChannelLinks ? 'left-5.5' : 'left-0.5'}`}></div>
                  </button>
                </div>
              </div>
            </div>
          ...
          <div className="space-y-8 lg:col-span-2">
            ...
            <div className="bg-app-card border border-app-ink/10 p-6 shadow-sm">
              <div className="flex items-center gap-3 mb-6">
                ...
                <h4 className="text-[11px] uppercase font-bold tracking-widest">Automation & Sync</h4>
              </div>
            ...
            <div className="space-y-6">
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2 opacity-60">
                    <RefreshCw size={14} />
                    <span className="text-[10px] font-bold uppercase tracking-tight">Background Auto-Sync</span>
                  </div>
                  <button 
                  ...
                  >
                    <div className={`absolute top-0.5 w-3.5 h-3.5 bg-white transition-all ${autoSyncEnabled ? 'left-5.5' : 'left-0.5'}`}></div>
                  </button>
                </div>
              ...
              <div className="space-y-4 pt-4 border-t border-app-ink/5">
                  ...
                  {globalStartTimeMode === "relative" && (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        />
                        <span className="text-[10px] opacity-60 uppercase tracking-widest font-bold">Days Ago</span>
                      </div>
                    </motion.div>
                  )}
                  {globalStartTimeMode === "absolute" && (
                    ...
                    >
                      <input
                        type="datetime-local"
                        value={typeof globalStartTimeValue === "string" ? globalStartTimeValue.slice(0, 16) : new Date().toISOString().slice(0, 16)}
                        onChange={(e) => {
                          if (e.target.value) {
                ...
                <div className="mt-2 p-2 bg-app-ink/5 border border-app-ink/10 rounded flex items-center justify-between">
                  <span className="text-[9px] uppercase font-bold opacity-40">Effective Start Date</span>
                ...
                {autoSyncEnabled && (
                  ...
                  >
                    <div className="flex items-center justify-between opacity-60">
                      <span className="text-[10px] font-bold uppercase tracking-tight">Sync Interval</span>
                      <span className="text-[10px] font-mono">{autoSyncInterval} Minutes</span>
                    </div>
                    <input 
                      type="range" 
                      min="5" 
          ...
          <>
            <div className="bg-app-card border border-app-ink/10 p-6 shadow-sm">
              <div className="flex items-center gap-3 mb-6">
                ...
                <h4 className="text-[11px] uppercase font-bold tracking-widest">Network & Proxy</h4>
              </div>
            ...
            <div className="space-y-6">
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2 opacity-60">
                    <Globe size={14} />
                    <span className="text-[10px] font-bold uppercase tracking-tight">Enable Proxies</span>
                  </div>
                  <button 
                  ...
                  >
                    <div className={`absolute top-0.5 w-3.5 h-3.5 bg-white transition-all ${proxyEnabled ? 'left-5.5' : 'left-0.5'}`}></div>
                  </button>
                </div>
                <p className="text-[10px] opacity-40 italic serif">Rotate through proxy servers to avoid Telegram rate limits.</p>
              </div>
              ...
              <AnimatePresence>
                {proxyEnabled && (
                  ...
                  >
                    <div className="flex items-center justify-between opacity-60">
                      <span className="text-[10px] font-bold uppercase tracking-tight">Proxy List (HTTP/SOCKS5)</span>
                      <div className="flex gap-3">
                        ...
                        >
                          {isTestingAll ? <RotateCw size={10} className="animate-spin" /> : <Activity size={10} />}
                          Test All
                        </button>
                      </div>
                    </div>
                        ...
                        {defaultProxyUrls.split(/[\n,]+/).map(p => p.trim()).filter(p => p && proxyTestResults[p]).map((url, idx) => {
                          ...
                          return (
                            <div key={idx} className="flex items-center justify-between text-[9px] bg-app-ink/5 p-2 border border-app-ink/5 rounded">
                              <span className="font-mono truncate max-w-[150px] opacity-60">{url}</span>
                              <div className="flex items-center gap-2">
                                {res.testing ? (
                                  <span className="flex items-center gap-1 opacity-40"><RotateCw size={8} className="animate-spin" /> Testing...</span>
                                ) : res.success ? (
                                  <>
                                    <span className="text-green-600 font-bold flex items-center gap-1"><CheckCircle2 size={8} /> {res.latency}ms</span>
                                    <span className="opacity-40 font-mono">({res.ip})</span>
                                  </>
                                ) : (
                                  <span className="text-red-600 font-bold flex items-center gap-1" title={res.error}><XCircle size={8} /> Error</span>
                                )}
                              </div>
                            </div>
                          );
                    ...
                    {badProxies.length > 0 && (
                      <div className="space-y-3 pt-2 border-t border-app-ink/5">
                        <div className="flex items-center justify-between">
                          ...
                          <span className="text-[8px] opacity-40 uppercase tracking-widest">Auto-Cooldown</span>
                        </div>
                        <div className="space-y-1">
                          {badProxies.map((proxy, idx) => (
                            <div key={idx} className="flex items-center justify-between text-[9px] bg-red-500/5 p-2 border border-red-500/10 rounded">
                              <span className="font-mono truncate max-w-[180px] opacity-60">{proxy.url}</span>
                              <span className="text-red-600 font-bold tabular-nums">
                                {Math.floor(proxy.cooldownRemaining / 60)}:{(proxy.cooldownRemaining % 60).toString().padStart(2, '0')}
                              </span>
                            </div>
                          ))}
                        </div>
                        <p className="text-[8px] opacity-30 italic serif">Proxies are temporarily avoided after repeated network failures.</p>
                      </div>
                    )}
          ...
          <div className="bg-app-card border border-app-ink/10 p-6 shadow-sm">
            ...
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-3">
                ...
                <h4 className="text-[11px] uppercase font-bold tracking-widest">TOR Network</h4>
              </div>
              <div className="flex items-center gap-2">
                <div className={`w-1.5 h-1.5 rounded-full ${torStatus?.running ? "bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.5)]" : "bg-red-500"}`} />
            ...
            <div className="space-y-6">
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2 opacity-60">
                    <Shield size={14} />
                    <span className="text-[10px] font-bold uppercase tracking-tight">Enable TOR Proxy</span>
                  </div>
                  <button 
                  ...
                  >
                    <div className={`absolute top-0.5 w-3.5 h-3.5 bg-white transition-all ${torEnabled ? 'left-5.5' : 'left-0.5'}`}></div>
                  </button>
                </div>
                <p className="text-[10px] opacity-40 italic serif">Use TOR to anonymize requests and bypass geographic restrictions.</p>
              </div>
                ...
                {torEnabled && (
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between bg-app-ink/5 p-4 rounded-xl border border-app-ink/10">
                      <div className="flex items-center gap-3">
                        <div className={`w-2 h-2 rounded-full ${torStatus?.running ? 'bg-green-500 animate-pulse' : 'bg-red-500'}`} />
                    ...
                    <div className="space-y-3">
                      <span className="text-[10px] font-bold uppercase tracking-tight opacity-60">Connection Mode</span>
                      <div className="flex bg-app-ink/5 p-1 rounded-lg border border-app-ink/10">
                        {(["auto", "custom"] as const).map((mode) => (
                          ...
                          >
                            {mode === "auto" ? "Automatic (Local)" : "Custom Cluster"}
                          </button>
                        ))}
                      </div>
                    ...
                    <AnimatePresence>
                      {torMode === "custom" && (
                        ...
                        >
                          <div className="flex items-center justify-between">
                            <span className="text-[10px] font-bold uppercase tracking-tight opacity-60">SOCKS5 Proxy Pool</span>
                            <div className="flex gap-3">
                              {Object.keys(proxyTestResults).length > 0 && (
                                <button 
                                  onClick={clearProxyResults}
                          ...
                          {Object.keys(proxyTestResults).length > 0 && torProxyUrls.split(/[\n,]+/).some(p => proxyTestResults[p.trim()]) && (
                            <div className="space-y-1 max-h-40 overflow-y-auto pr-2 custom-scrollbar">
                              {torProxyUrls.split(/[\n,]+/).map(p => p.trim()).filter(p => p && proxyTestResults[p]).map((url, idx) => {
                                const res = proxyTestResults[url];
                                // For TOR pool, we might need to prepend socks5h:// if it's just IP:PORT
                                const displayUrl = url.includes('://') ? url : `socks5h://${url}`;
                                return (
                                  <div key={idx} className="flex items-center justify-between text-[9px] bg-app-ink/5 p-2 border border-app-ink/5 rounded">
                                    <span className="font-mono truncate max-w-[150px] opacity-60">{url}</span>
                                    <div className="flex items-center gap-2">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                      <div className="space-y-3">
                        <span className="text-[10px] font-bold uppercase tracking-tight opacity-60">Rotation Strategy</span>
                        <div className={`flex bg-app-ink/5 p-1 rounded-lg border border-app-ink/10 ${torMode === 'auto' ? 'opacity-30 grayscale pointer-events-none' : ''}`}>
                          {(["sequential", "random"] as const).map((strategy) => (
                            <button
                              key={strategy}
                      ...
                      <div className="space-y-3">
                        <span className="text-[10px] font-bold uppercase tracking-tight opacity-60">Quick Actions</span>
                        <div className="flex gap-2">
                          <button 
                            onClick={restartTor}
                          ...
                          >
                            {isCheckingIp ? <RefreshCw size={10} className="animate-spin" /> : <Globe size={10} />}
                    ...
                    <AnimatePresence>
                      {torIp && (
                        ...
                        >
                          <span className="text-[9px] uppercase font-bold opacity-40">Current Exit IP</span>
                          <span className="text-[10px] font-mono font-bold tracking-wider">{torIp}</span>
                        </motion.div>
                      )}
                    </AnimatePresence>
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2 opacity-60">
                          ...
                          <div className="flex flex-col">
                            <span className="text-[10px] font-bold uppercase tracking-tight">IP Rotation Control</span>
                            <span className="text-[8px] opacity-40 italic serif">Requires ControlPort enabled in torrc</span>
                          </div>
                        </div>
                        <button 
                        ...
                        >
                          <div className={`absolute top-1 w-3 h-3 rounded-full bg-app-bg transition-all ${torControlEnabled ? 'left-6' : 'left-1'}`} />
                        </button>
                      </div>
                        ...
                        {torControlEnabled && (
                          ...
                          >
                            <div className="grid grid-cols-1 gap-4">
                              <div className="space-y-2">
                                ...
                                <input 
                                  ...
                                  onChange={(e) => setTorControlPort(parseInt(e.target.value))}
                            ...
                            <div className="flex items-center justify-between pt-2 bg-app-ink/5 p-3 rounded-lg border border-app-ink/10">
                              ...
                              <button 
                                onClick={changeTorIp}
                                disabled={isChangingIp || !torStatus?.controlInUse}
                              ...
                              >
                                {isChangingIp ? <RotateCw size={10} className="animate-spin" /> : <Shield size={10} />}
                            ...
                            <div className="pt-4 border-t border-app-ink/5 space-y-4">
                              <div className="flex items-center justify-between">
                                <div className="flex flex-col">
                                  <span className="text-[10px] font-bold uppercase tracking-tight opacity-60">Auto-Rotate IP</span>
                                  <span className="text-[8px] opacity-40 italic serif">Rotate identity automatically after X requests.</span>
                                </div>
                                <button
                                ...
                                >
                                  <div className={`absolute top-1 w-3 h-3 rounded-full bg-app-bg transition-all ${torAutoRotate ? 'left-6' : 'left-1'}`} />
                                </button>
                              </div>
                              ...
                              {torAutoRotate && (
                                ...
                                >
                                  <div className="flex flex-col">
                                    <span className="text-[10px] font-bold uppercase tracking-tight opacity-60">Rotation Threshold</span>
                                    <span className="text-[8px] opacity-40 italic serif">Requests before rotation.</span>
                                  </div>
                                  <div className="flex items-center gap-3">
                                    <span className="text-[10px] font-mono font-bold">{torRotationThreshold}</span>
                                    <input 
                                      type="range" 
                                      min="5" 
                                      max="50" 
                                      ...
                                      onChange={(e) => setTorRotationThreshold(parseInt(e.target.value))}
        ...
        {activeSection === "ai" && (
          <div className="space-y-8 lg:col-span-2">
            ...
            <div className="bg-app-card border border-app-ink/10 p-6 shadow-sm">
              <div className="flex items-center gap-3 mb-6">
                ...
                <h4 className="text-[11px] uppercase font-bold tracking-widest">AI Engine Parameters</h4>
              </div>
            ...
            <div className="space-y-8">
              <div className="space-y-4">
                <div className="flex items-center gap-2 opacity-60">
                  <Languages size={14} />
                  <span className="text-[10px] font-bold uppercase tracking-tight">Output Language</span>
                </div>
                <select
                  value={aiLanguage}
                ...
                >
                  {LANGUAGES.map(l => (
                    <option key={l} value={l} className="bg-app-card text-app-ink">{l.toUpperCase()}</option>
                  ))}
                </select>
                <p className="text-[10px] opacity-40 italic serif">Summaries and chat responses will be generated in this language.</p>
              </div>

              <div className="space-y-4">
                <div className="flex items-center gap-2 opacity-60">
                  <Cpu size={14} />
                  <span className="text-[10px] font-bold uppercase tracking-tight">Default Model</span>
                </div>
                <select
                  value={selectedModel}
                ...
                >
                  {MODELS.map(m => (
                    <option key={m.id} value={m.id} className="bg-app-card text-app-ink">{m.label.toUpperCase()}</option>
                  ))}
                </select>
                <p className="text-[10px] opacity-40 italic serif">Flash models are faster, Pro models are more detailed.</p>
              </div>
                ...
                <>
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2 opacity-60">
                        <Cpu size={14} />
                        <span className="text-[10px] font-bold uppercase tracking-tight">Enable Embeddings & RAG</span>
                      </div>
                      <button 
                      ...
                      >
                        <div className={`absolute top-0.5 w-3.5 h-3.5 bg-white transition-all ${embeddingsEnabled ? 'left-5.5' : 'left-0.5'}`}></div>
                      </button>
                    </div>
                    <p className="text-[10px] opacity-40 italic serif">Generate vector embeddings for posts to enable semantic search and RAG chat. Warning: Takes significant disk space.</p>
                  </div>
                  ...
                  {embeddingsEnabled && (
                    <div className="space-y-4">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2 opacity-60">
                          <Database size={14} />
                          <span className="text-[10px] font-bold uppercase tracking-tight">Pause Embedding Sync</span>
                        </div>
                        <button 
                        ...
                        >
                          <div className={`absolute top-0.5 w-3.5 h-3.5 bg-white transition-all ${embeddingsPaused ? 'left-5.5' : 'left-0.5'}`}></div>
                        </button>
                      </div>
                      <p className="text-[10px] opacity-40 italic serif">Temporarily pause the background generation of embeddings to save API quota or CPU usage.</p>
                    </div>
                  )}
                </>
              )}
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between opacity-60">
                  <div className="flex items-center gap-2">
                    <Thermometer size={14} />
                    <span className="text-[10px] font-bold uppercase tracking-tight">Creativity (Temperature)</span>
                  </div>
                  <span className="text-[10px] font-mono">{aiTemperature.toFixed(1)}</span>
                </div>
                <input 
                  type="range" 
                  min="0" 
          ...
          <div className="bg-app-card border border-app-ink/10 p-6 shadow-sm mt-8">
            <div className="flex items-center gap-3 mb-6">
              ...
              <h4 className="text-[11px] uppercase font-bold tracking-widest">Translation Engine</h4>
            </div>
            ...
            <div className="space-y-8">
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2 opacity-60">
                    <Languages size={14} />
                    <span className="text-[10px] font-bold uppercase tracking-tight">Enable Translation</span>
                  </div>
                  <button 
                  ...
                  >
                    <div className={`absolute top-0.5 w-3.5 h-3.5 bg-white transition-all ${translationEnabled ? 'left-5.5' : 'left-0.5'}`}></div>
                  </button>
                </div>
                <p className="text-[10px] opacity-40 italic serif">Allow translating posts natively using Gemini.</p>
              </div>
                  ...
                  >
                    <div className="space-y-4">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2 opacity-60">
                          <RefreshCw size={14} />
                          <span className="text-[10px] font-bold uppercase tracking-tight">Auto-Translate Posts</span>
                        </div>
                        <button 
                        ...
                        >
                          <div className={`absolute top-0.5 w-3.5 h-3.5 bg-white transition-all ${autoTranslate ? 'left-5.5' : 'left-0.5'}`}></div>
                        </button>
                      </div>
                      <p className="text-[10px] opacity-40 italic serif">Automatically translate posts when they are loaded.</p>
                    </div>

                    <div className="space-y-4">
                      <div className="flex items-center gap-2 opacity-60">
                        <Languages size={14} />
                        <span className="text-[10px] font-bold uppercase tracking-tight">Target Language</span>
                      </div>
                      <select
                    ...
                    <div className="space-y-4">
                      <div className="flex items-center gap-2 opacity-60">
                        <Cpu size={14} />
                        <span className="text-[10px] font-bold uppercase tracking-tight">Translation Model</span>
                      </div>
                      <select
