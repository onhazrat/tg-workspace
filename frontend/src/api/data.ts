...
export const dataApi = {
  ...
  deleteChannel: (id: string) =>
    request<{ status: string }>(`/api/v1/data/channels/${id}`, { method: "DELETE" }),

  getChannelStats: (id: string) =>
    request<ChannelStats>(`/api/v1/data/channels/${id}/stats`),

  getPosts: (params?: { channelNames?: string[]; startDate?: number; endDate?: number }) => {
    const qs = new URLSearchParams();
    if (params?.channelNames?.length) qs.set("channelNames", params.channelNames.join(","));
  ...
  deleteSummary: (id: string) =>
    request<{ status: string }>(`/api/v1/data/summaries/${id}`, { method: "DELETE" }),

  listBotCredentials: () => request<BotCredential[]>("/api/v1/data/bot-credentials"),
  ...
  deleteBotCredential: (id: string) =>
    request<{ status: string }>(`/api/v1/data/bot-credentials/${id}`, { method: "DELETE" }),

  migrateBotCredentials: (bots: BotCredential[]) =>
    request<{ migrated: number; ids: string[] }>("/api/v1/data/bot-credentials/migrate", {
      method: "POST",
      body: JSON.stringify(bots),
    }),

  getNetworkSettings: () =>
    request<{ key: string; value: Record<string, unknown> }>("/api/v1/data/settings/network"),
  ...
  deleteChatDestination: (id: string) =>
    request<{ status: string }>(`/api/v1/data/chat-destinations/${id}`, { method: "DELETE" }),
  ...
  listTranslations: () => request<PostTranslation[]>("/api/v1/data/translations"),
  ...
  listEmbeddingLogs: () => request<EmbeddingLog[]>("/api/v1/data/embedding-logs"),
