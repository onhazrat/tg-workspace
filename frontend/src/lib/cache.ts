...
import { Post, Channel, BotCredential, ChatDestination, Summary, PublishLog, SyncLog, LLMLog, EmbeddingLog, NetworkLog, PostEmbedding, PostTranslation } from '../types';
  ...
  dbPromise = (async () => {
    ...
    return openDB(DB_NAME, DB_VERSION, {
      upgrade(db, oldVersion, newVersion, transaction) {
        ...
        if (oldVersion < 1) {
          const postStore = db.createObjectStore(STORE_POSTS, { keyPath: ['channelName', 'id'] });
        ...
        if (oldVersion < 6) {
          if (!db.objectStoreNames.contains(STORE_PUBLISH_LOGS)) {
            const logStore = db.createObjectStore(STORE_PUBLISH_LOGS, { keyPath: 'id' });
        ...
        if (oldVersion < 7) {
          if (!db.objectStoreNames.contains(STORE_SYNC_LOGS)) {
            const syncLogStore = db.createObjectStore(STORE_SYNC_LOGS, { keyPath: 'id' });
        ...
        if (oldVersion < 8) {
          if (!db.objectStoreNames.contains(STORE_LLM_LOGS)) {
            const llmLogStore = db.createObjectStore(STORE_LLM_LOGS, { keyPath: 'id' });
        ...
        if (oldVersion < 9 && transaction) {
          ...
          if (!postStore.indexNames.contains('channelName_timestamp')) {
            postStore.createIndex('channelName_timestamp', ['channelName', 'timestamp']);
          }
        }
        ...
        if (oldVersion < 11) {
          if (!db.objectStoreNames.contains(STORE_EMBEDDING_LOGS)) {
            const embeddingLogStore = db.createObjectStore(STORE_EMBEDDING_LOGS, { keyPath: 'id' });
            embeddingLogStore.createIndex('timestamp', 'timestamp');
          }
          if (!db.objectStoreNames.contains(STORE_NETWORK_LOGS)) {
            const networkLogStore = db.createObjectStore(STORE_NETWORK_LOGS, { keyPath: 'id' });
        ...
        if (oldVersion < 12) {
          if (!db.objectStoreNames.contains(STORE_EMBEDDINGS)) {
            const embeddingStore = db.createObjectStore(STORE_EMBEDDINGS, { keyPath: 'id' });
...
export async function migrateEmbeddingsData() {
  ...
  try {
    ...
    console.log(`[DB] Embeddings migration complete. Migrated ${migratedCount} embeddings.`);
  } catch (error) {
...
export async function migrateSummaryDates() {
  ...
  try {
    ...
    await tx.done;
    console.log(`[DB] Summary dates migration complete. Migrated ${migratedCount} summaries.`);
...
export async function getTranslation(channelName: string, postId: number, language: string): Promise<PostTranslation | undefined> {
  const db = await initDB();
...
export async function getPostsByDateRange(channels: string[], startDate: number, endDate: number) {
  if (startDate > endDate) {
    console.warn('[DB] getPostsByDateRange: startDate > endDate, returning empty results', { startDate, endDate });
    return [];
  }
  ...
  if (store.indexNames.contains('channelName_timestamp')) {
    ...
    for (const channelName of channels) {
      const range = IDBKeyRange.bound([channelName, startDate], [channelName, endDate]);
    ...
    results.push(...allPosts.filter(post => channels.includes(post.channelName)));
  }
...
export async function getChannelStats(channelName: string) {
  ...
  let maxCursor = await index.openCursor(IDBKeyRange.only(channelName), 'prev');
  if (maxCursor) {
    maxId = maxCursor.value.id;
  }

  const minCursor = await index.openCursor(IDBKeyRange.only(channelName), 'next');
  if (minCursor) {
  ...
  if (count >= 2) {
    ...
    if (timestamps.length >= 2) {
      ...
      const timeSinceLast = (Date.now() - timestamps[timestamps.length - 1]) / (1000 * 60 * 60);
...
export async function getPostsChunk(offset: number, limit: number): Promise<Post[]> {
  const db = await initDB();
  const tx = db.transaction(STORE_POSTS, 'readonly');
  const store = tx.objectStore(STORE_POSTS);
  const results: Post[] = [];
...
export async function importDB(data: { data: { posts?: Post[], channels?: Channel[], summaries?: Summary[], bot_credentials?: BotCredential[], chat_destinations?: ChatDestination[], publish_logs?: PublishLog[], sync_logs?: SyncLog[], llm_logs?: LLMLog[], embedding_logs?: EmbeddingLog[], network_logs?: NetworkLog[], localStorage?: Record<string, string> } }, clear: boolean = true) {
  const db = await initDB();
  
  const storesToUpdate = [];
  if (db.objectStoreNames.contains(STORE_POSTS)) storesToUpdate.push(STORE_POSTS);
  if (db.objectStoreNames.contains(STORE_CHANNELS)) storesToUpdate.push(STORE_CHANNELS);
  if (db.objectStoreNames.contains(STORE_SUMMARIES)) storesToUpdate.push(STORE_SUMMARIES);
  if (db.objectStoreNames.contains(STORE_BOT_CREDENTIALS)) storesToUpdate.push(STORE_BOT_CREDENTIALS);
  if (db.objectStoreNames.contains(STORE_CHAT_DESTINATIONS)) storesToUpdate.push(STORE_CHAT_DESTINATIONS);
  if (db.objectStoreNames.contains(STORE_PUBLISH_LOGS)) storesToUpdate.push(STORE_PUBLISH_LOGS);
  if (db.objectStoreNames.contains(STORE_SYNC_LOGS)) storesToUpdate.push(STORE_SYNC_LOGS);
  if (db.objectStoreNames.contains(STORE_LLM_LOGS)) storesToUpdate.push(STORE_LLM_LOGS);
  if (db.objectStoreNames.contains(STORE_EMBEDDING_LOGS)) storesToUpdate.push(STORE_EMBEDDING_LOGS);
  if (db.objectStoreNames.contains(STORE_NETWORK_LOGS)) storesToUpdate.push(STORE_NETWORK_LOGS);
  ...
  if (storesToUpdate.length > 0) {
    ...
    if (data.data.bot_credentials && storesToUpdate.includes(STORE_BOT_CREDENTIALS)) {
    ...
    if (data.data.chat_destinations && storesToUpdate.includes(STORE_CHAT_DESTINATIONS)) {
    ...
    if (data.data.embedding_logs && storesToUpdate.includes(STORE_EMBEDDING_LOGS)) {
...
export async function clearAllData() {
  ...
  if (db.objectStoreNames.contains(STORE_CHANNELS)) storesToClear.push(STORE_CHANNELS);
  if (db.objectStoreNames.contains(STORE_SUMMARIES)) storesToClear.push(STORE_SUMMARIES);
  if (db.objectStoreNames.contains(STORE_BOT_CREDENTIALS)) storesToClear.push(STORE_BOT_CREDENTIALS);
  if (db.objectStoreNames.contains(STORE_CHAT_DESTINATIONS)) storesToClear.push(STORE_CHAT_DESTINATIONS);
  if (db.objectStoreNames.contains(STORE_PUBLISH_LOGS)) storesToClear.push(STORE_PUBLISH_LOGS);
  if (db.objectStoreNames.contains(STORE_SYNC_LOGS)) storesToClear.push(STORE_SYNC_LOGS);
  if (db.objectStoreNames.contains(STORE_LLM_LOGS)) storesToClear.push(STORE_LLM_LOGS);
  if (db.objectStoreNames.contains(STORE_EMBEDDING_LOGS)) storesToClear.push(STORE_EMBEDDING_LOGS);
  if (db.objectStoreNames.contains(STORE_NETWORK_LOGS)) storesToClear.push(STORE_NETWORK_LOGS);
  if (db.objectStoreNames.contains(STORE_TRANSLATIONS)) storesToClear.push(STORE_TRANSLATIONS);
...
export async function getTableSizes(): Promise<{ name: string; size: number; count: number }[]> {
  const db = await initDB();
...
export async function runQuery(storeName: string, query: string): Promise<any[]> {
  const db = await initDB();
  ...
  try {
    ...
    const filterFn = new Function('record', `
      try {
        with(record) {
          return ${query};
        }
...
export async function deleteOldPosts(days: number): Promise<number> {
  ...
  const tx = db.transaction([STORE_POSTS, STORE_EMBEDDINGS, STORE_TRANSLATIONS], 'readwrite');
  const postStore = tx.objectStore(STORE_POSTS);
  const embeddingStore = tx.objectStore(STORE_EMBEDDINGS);
  ...
  while (cursor) {
    ...
    let transCursor = await translationIndex.openCursor(IDBKeyRange.only(post.id));
    while (transCursor) {
...
export async function deleteOldLogs(days: number): Promise<number> {
  ...
  const existingStores = logStores.filter(store => db.objectStoreNames.contains(store));
