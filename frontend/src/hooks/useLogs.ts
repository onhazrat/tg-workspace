...
import type { EmbeddingLog, LLMLog, NetworkLog, PublishLog, SyncLog } from "@/types";
...
export function useLogsQuery(type: LogType, enabled = false) {
  return useQuery({
    ...
    queryFn: async () => sortByTimestamp((await fetchers[type]()) as { timestamp: number }[]),
...
export function usePublishLogsQuery(enabled = false) {
  return useLogsQuery("publish", enabled) as ReturnType<typeof useQuery<PublishLog[]>>;
}
...
export function useEmbeddingLogsQuery(enabled = false) {
  return useLogsQuery("embedding", enabled) as ReturnType<typeof useQuery<EmbeddingLog[]>>;
}

export function useNetworkLogsQuery(enabled = false) {
  return useLogsQuery("network", enabled) as ReturnType<typeof useQuery<NetworkLog[]>>;
}
