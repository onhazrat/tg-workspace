...
export const ScraperProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const { channels, selectedChannels, setChannelStats, loadChannels, loadSyncLogs } = useData();
  ...
  const runServerSync = useCallback(async (
    ...
    if (isOffline) {
      toast.warning("Server offline — sync disabled. Browsing cached data only.");
      return;
    }
    if (channelIds.length === 0) return;

    console.log(`[Scraper] Starting server sync for ${channelIds.length} channel(s) from ${source}`);
    ...
    try {
      ...
      if (failures.length > 0) {
        ...
        if (failures.length === 1) {
          toast.error(`Sync failed for @${failures[0].channelName}: ${firstErr}`);
        } else {
  ...
  const scrapeChannelsInParallel = async (channelsToScrape: Channel[], source: string) => {
    ...
    console.log(`[SyncQueue] Starting server job for ${active.length} channels from ${source}`);
    await runServerSync(
      active.map(c => c.id),
      active.map(c => c.name),
      source,
